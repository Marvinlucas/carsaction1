...
// Check if the seller's profile picture is empty (no profile picture)
if (empty($sellerProfilePicture)) {
    // If there's no profile picture, use the default image path
    $defaultProfilePicture = "include/image/noprofile.jpg";
    $sellerProfilePicture = $defaultProfilePicture;
}
...
echo "<img src='$sellerProfilePicture' alt='Profile Picture' style='width: 100px; height: 100px; border-radius: 50%; object-fit: cover; margin-right: 20px;'>"; // Add profile picture
echo "<div>"; // Create a div for seller's name
echo "<h1 class='card-text' style='font-size: 20px; color:#007bff'>$userFirstName $userLastName</h1>";
echo "<h3 class='card-text' style='font-size: 20px; color:#00000f'>Location: $userLocation</h3>";
echo "<a href='revViewprofile.php?sellers_id=" . $row['sellers_id'] . "' >View Profile</a>";
echo "</div>"; // Close the div for seller's name
echo "</div>"; // Close the flex container


 if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
                echo "<a href='userLogin.php?car_id=$carId' class='btn btn-primary'>Message</a>";
            } else {
                echo "<a href='userLogin.php' class='btn btn-primary'>Message</a>";
            }
